...
class Firework:
    ...
    def update(self):
        if not self.exploded: # Di chuyển pháo hoa lên trên màn hình
            ...
            if self.y <= random.randint(100, 300): # Khi pháo hoa về một độ cao nhất định, nổ
                self.explode()
        else:
...
class Particle:
    def __init__(self, x, y, color):
        ...
        self.y = y
        self.color = color
        self.speed_x = random.uniform(-3, 3) # Tốc độ ngẫu nhiên theo chiều ngang
        self.speed_y = random.uniform(-12, -6) # Tốc độ ngẫu nhiên theo chiều dọc
        self.size = random.uniform(1, 4) # Kích thước ngẫu nhiên
...
while running:
    ...
    if random.random() < 0.1: # Một cơ hội nhỏ để tạo ra một pháo hoa mới
        firework = Firework()
